import { getDatabase, ref, push, get, set, update, onValue, remove, off,
  query, limitToFirst, limitToLast, orderByChild, equalTo
    ...
    if (imageFiles.length > 0) {
      ...
      const requests = imageFiles.map((image) => {
        if (typeof image.source === 'string') {
          ...
          return instance.post(`https://api.cloudinary.com/v1_1/${process.env.NEXT_PUBLIC_CLOUDINARY_NAME}/image/upload`, formData, {
...
export const getTrolleysDataRef = () => {
  let nodeRef = query(ref(database, '/trolley_data'));
  return nodeRef;
}

export const getTrolleyDataRef = (trolleyId) => {
  let nodeRef = query(ref(database, `/trolley_data/${trolleyId}`));
  return nodeRef;
}

export const getTrolleyConfigRef = (trolleyId) => {
  let nodeRef = query(ref(database, `/trolley_config/${trolleyId}`));
  return nodeRef;
}

export const getTrolleyHistoryRef = (trolleyId, quantity=5) => {
  let nodeRef = query(ref(database, `/trolley_past_data/${trolleyId}`), limitToLast(quantity), orderByChild('dateCreated'));
  return nodeRef;
}

export const getChatsRef = () => {
  let nodeRef = query(ref(database, "/customer_chats/"));
  return nodeRef;
}

export const addChatMessage = async (userID, data, successHandler, errorHandler) => {
  const currentTimeMillis = new Date().getTime();
  const newChildRef = push(ref(database, `/customer_chats/${userID}/messages/`));
...
export const deleteAllChatMessages = async (userID, successHandler, errorHandler) => {
  const nodeRef = ref(database, `/customer_chats/${userID}/messages/`);
  remove(nodeRef).then(successHandler).catch(errorHandler);
}
